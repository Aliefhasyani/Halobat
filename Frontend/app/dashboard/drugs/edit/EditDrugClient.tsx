...
export default function EditDrugClient() {
        ...
        if (json.success && json.data) {
          ...
          if (Array.isArray(d.active_ingredients))
            setActiveIngredientsList(
              d.active_ingredients.map((a: Record<string, unknown>) => ({
                id: String(
                ...
                quantity: Number(
                  (a.quantity ??
                    a.qty ??
                    (a.pivot &&
                      (a.pivot as Record<string, unknown>).quantity) ??
                    0) as number
                ),
              }))
            );
          drugLoadedRef.current = true;
        } else setError(json.error || "Failed to load drug");
      })
      .catch((e) => {
        console.error(e);
  ...
  // file upload handler — mirrors Create page behaviour: sets the `picture` field
